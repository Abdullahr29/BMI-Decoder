...
[params] = get_lin_regression_params(training_data,L ,fr_total);

toc
t = toc;

%% prediction
function [x_pos,y_pos] = predict(testing_data,pred_angle, model)
   idx_bin = length(obj.fr_total)/98-(320/dt-1);
   x_pos = model(pred_angle,idx_bin).x_avg_sampled;
   y_pos = model(pred_angle,idx_bin).y_avg_sampled;
   
end


%% Functions
function [model] = get_lin_regression_params(data,L,fr_total)
% data: training data
